...
const en = {
  ...
  //guida biblioteca
  libraryGuide: {
    backNavTitle: "Library guide",
    guide: [
      { main: "APRIRE LA BIBLIOTECA:  Una volta entrati nella schermata del proprio gruppo, posizionarsi nella navbar in basso e fare click sull’ultima icona “Biblioteca” (ultima icona a destra a forma di libro). Si aprirà il catalogo dei libri messi a disposizione dagli utenti del gruppo.", secondary: "" },
      { main: "AGGIUNGERE UN LIBRO AL CATALOGO: All’interno della schermata catalogo è presente un pulsante tondo con simbolo + in basso a destra,e successivamente sul pulsante “Aggiungi libro” che comparirà. Si aprirà una schermata dove si potranno inserire titolo, autore e descrizione del libro, e sarà inoltre possibile caricare l’immagine di copertina, che verrà poi visualizzata all’interno del catalogo. (ATTENZIONE: caricare l’immagine è facoltativo, inoltre la dimensione massima supportata per le immagini è di 50 MB). Infine fare click su conferma per completare l’operazione. ", secondary: "" },
      { main: "SCHERMATA INFORMAZIONI  LIBRO:  Cliccando sul  titolo del libro nel catalogo si aprirà una finestra con le relative informazioni. Le informazioni visualizzate in questa pagina sono diverse a seconda dell’utente loggato: \na.        Se si è il proprietario del libro visualizzato è possibile: Modificare le informazioni del libro: è sufficiente fare click sul tasto “edit”, modificare le informazioni e successivamente fare click su “conferma”. Il bottone conferma non reindirizza alla pagina precedente ma è necessario premere manualmente il tasto indietro Eliminare il libro: fare click sul tasto “delete”. Visualizzare la coda delle richieste di prestito per quel libro: si avrà la possibilità di accettare o rifiutare la richiesta che risulta per prima in coda (per le richieste successive è abilitato solo il tasto rifiuta: il proprietario infatti può accettare solo la prima richiesta in coda, questo per garantire che ciascun libro sia preso in prestito da un utente per volta.)\n b.       Se non si è proprietari del libro visualizzato è possibile:  Visualizzare le informazioni del libro, senza il privilegio di modificarle.Visualizzare lo stato del libro: sopra alle informazioni, in alto a sinistra, comparirà “Disponibile” se il libro non è attualmente in prestito e “Non disponibile” se qualche altro utente ha già preso il libro. Prenotare il libro, facendo click sull’apposito pulsante “Prenota”. In caso il libro sia già stata prenotato non verrà creato un altro prestito.", secondary: "" },
      { main: "PRENOTAZIONE DI UN LIBRO: Dopo che un utente ha prenotato il libro, cliccando su “prenota” (vedi il punto 3b) \na.	il suo proprietario potrà visualizzare la nuova richiesta nella coda, all’interno della pagina informazioni libro (vedi il punto 3a). La richiesta sarà dunque in attesa di accettazione. Se il prestito è il primo della coda il proprietario potrà accettarlo facendo click su “accetta” e successivamente selezionando il termine del prestito dal calendario che comparirà, altrimenti si dovrà aspettare il turno dell’utente in questione. \nb. Una volta che il proprietario avrà accettato il prestito, può cliccare sopra all’indirizzo mail del richiedente (presente nella lista di prenotazioni da lui visibile, vedi sezione 3a) dove si aprirà un link al gestore email predefinito, e si potrà quindi contattare l’utente che ha richiesto il prestito, in modo da prendere accordi per lo scambio fisico del libro. \n\nrietario rifiuti il prestito, la richiesta dell’utente verrà eliminata dalla coda, e l’utente richiedente non potrà ricevere il libro. Se vorrà però potrà sempre effettuare un altra richiesta in un momento successivo: il rifiuto a una richiesta non è vincolante, e l’utente richiedente non è in alcun modo “bloccato”, può continuare ad effettuare richieste, anche per lo stesso libro per cui ha ricevuto il rifiuto. L’utente che aveva richiesto il prestito ma è stato annullato lo potrà vedere nella apposita pagina “I miei libri” ( vedi punto 5 ) \n\nc. Una volta che l’utente ha terminato la lettura del libro, può contattare il proprietario per prendere accordi sulla restituzione del libro ( l’indirizzo email del proprietario è presente nella schermata informazioni del libro relativo, e anch’esso contiene un link al gestore email predefinito, in modo da automatizzare la scrittura delle mail). \nd.Dopo che il libro è stato restituito al proprietario, esso dovrà fare click sul bottone “conferma restituzione“ presente sulla lista di prenotazioni, in modo da eliminare il prestito corrente dalla coda e rendere nuovamente il libro disponibile per l’utente successivo.", secondary: "" },
      { main: "PAGINA “I MIEI LIBRI” \nDalla schermata iniziale del gruppo, cliccando sul pulsante con icona “+” e successivamente sul pulsante “I miei libri” che comparirà, si potrà accedere a una schermata in cui sarà possibile visualizzare i propri libri messi a disposizione con il relativo stato (prenotato/non prenotato) le proprie prenotazioni effettuate, con la relativa scadenza.  \nOltretutto nella sezione prenotazione si potrà annullare la prenotazione o visualizzare le prenotazioni annullate, accettate o non.  \nEventuali prenotazioni annullate indicano che il prestito è stato cancellato. Per poter chiedere un nuovo prestito su quel libro bisogna confermare sul relativo tasto “ok” dell’avvenuta visualizzazione dell’annullamento", secondary: "" },
